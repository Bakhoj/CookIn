...
public class FireBHandler {
    ...
    public String uploadDinner(Banquet banquet) {
        /* Set the com.cookin.dinners' id to null, making sure the dinner will be a new one in the database */
        banquet.setDinnerId(null);
        /* Set the rightful owner to the Dinner,
        *  only the Authenticated user is allowed to make com.cookin.dinners for themself */
     ...
     * will download all the com.cookin.dinners associated with the given userId
     * @param facebookId - The userId of the com.cookin.dinners
     * @return a List of all the com.cookin.dinners the given user has
     */
    public List<Banquet> downloadAllDinnersFrom(String facebookId) {
        /* The List of com.cookin.dinners to be returned in the end of the method*/
        final List<Banquet> banquetList = new ArrayList<>();
        Log.i("FireBHandler", "downloadAllDinnersFrom '" + facebookId + "' has started");
        /* Set the database reference to com.cookin.dinners */
        ...
        queryRef.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot snapshot) {
                ...
                /* get all the children(com.cookin.dinners) in the com.cookin.dinners database*/
                Iterable<DataSnapshot> snapshotChildren = snapshot.getChildren();
                DataSnapshot newDataSnapshot;

                /* Run for all the children */
                for (int i = 0; i < childrenCount; i++) {
                    /* Get the next child (starts from nothing) of com.cookin.dinners */
                    ...
                    /* Get and set all the dinner com.cookin.data */
     ...
     * will download all com.cookin.dinners except from the userId given,
     * the com.cookin.dinners will be sorted by startDate of the com.cookin.dinners
     * @param facebookId - The given userId
     * @return a List of all the com.cookin.dinners from everyone else then @facebookId
     */
